...
{
    [TestClass]
    public class GreedySolverWithRealFlightTests
    {
        private ISolver CreateCheapestRouteSolver()
        {
            var flightSearchService = new FlightSearchWebService("AUD", "Economy");
        ...
        {
            var solver = CreateCheapestRouteSolver();
        ...
        public void Test3CitiesCheapest()
        {
            ...
            Assert.IsTrue(result.TotalWaitingInterval > TimeSpan.Zero);
        }
        ...
        [TestMethod]
        public void Test5CitiesFastest()
        {
            var solver = CreateFastestRouteSolver();
            var cities = new string[] { "BKK", "BRU", "LHR", "SIN", "HKG" };
